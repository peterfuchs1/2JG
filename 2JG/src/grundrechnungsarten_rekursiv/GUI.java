...
public class GUI extends JFrame implements ActionListener{
		...
		if(e.getSource()==jb_start){
			...
			for(int i=0;i<anzahlVariable;i++){
				...
				if(!str.equals("")){
					...
					v1=
						new Variable<Integer>(wert,true);
				} 
				else {
					v1 =
						new Variable<Integer>(start,stopp,wert,false);				
				}
				v.add(i, v1);
			}
			// ERgebnis übernehmen
	...
	public boolean berechne(int vIndex){
		...
		Integer sta=v1.getStart();
		Integer sto=v1.getStopp();
		boolean f=v1.isFix();
//	  for(int i=v[vIndex].getStart();i<=v[vIndex].getStopp();i++) {
...
//			}
		...
		// Datensatz in ArrayList speichern
			...
			for(int i=0;i<s.length();i++){
				 v1=v.get(i);
//				 int help=v[i].getValue();
//				Integer help2=new Integer(help);
				s.setVal(i, v1.getValue());
			}
			sol.add(s);
